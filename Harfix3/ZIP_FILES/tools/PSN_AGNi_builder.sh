...
AGNI_LOG_FILE="/data/.harfix/log.txt"
...
if [ -d /tmp/extracted/res ] || [ -f /tmp/extracted/sbin/busybox ] || [ "`ls /tmp/extracted/sbin | grep .sh`" ];
	then
	###charger begin
	mkdir -p /tmp/extracted/res-TEMP
	cp /tmp/extracted/res/images/charger/* /tmp/extracted/res-TEMP/
	rm -rf /tmp/extracted/res/*
	mkdir -p /tmp/extracted/res/images/charger
	cp /tmp/extracted/res-TEMP/* /tmp/extracted/res/images/charger/
	rm -rf /tmp/extracted/res-TEMP
	###charger end
	...
	cp /tmp/extracted/sbin/mkfs.f2fs /tmp/extracted/sbin-TEMP
	rm -rf /tmp/extracted/sbin/*
	cp -r /tmp/extracted/sbin-TEMP/* /tmp/extracted/sbin/
...
if ! [ "`grep f2fs /tmp/extracted/lpm.rc`" ];
	then
	awk '/by-name/SYSTEM/{print "    mount f2fs /dev/block/platform/dw_mmc/by-name/SYSTEM /system ro wait noatime nodiratime inline_xattr active_logs=2"} {print}' /tmp/extracted/lpm.rc > /tmp/extracted/lpm.rc-temp
	mv /tmp/extracted/lpm.rc-temp /tmp/extracted/lpm.rc
	awk '/mmcblk0p9/{print "    mount f2fs /dev/block/platform/dw_mmc/by-name/SYSTEM /system ro wait noatime nodiratime inline_xattr active_logs=2"} {print}' /tmp/extracted/lpm.rc > /tmp/extracted/lpm.rc-temp
	mv /tmp/extracted/lpm.rc-temp /tmp/extracted/lpm.rc
	awk '/USERDATA/{print "    mount f2fs /dev/block/platform/dw_mmc/by-name/USERDATA /data wait noatime nosuid nodev nodiratime inline_xattr active_logs=2"} {print}' /tmp/extracted/lpm.rc > /tmp/extracted/lpm.rc-temp
	mv /tmp/extracted/lpm.rc-temp /tmp/extracted/lpm.rc
	awk '/mmcblk0p12/{print "    mount f2fs /dev/block/platform/dw_mmc/by-name/USERDATA /data wait noatime nosuid nodev nodiratime inline_xattr active_logs=2"} {print}' /tmp/extracted/lpm.rc > /tmp/extracted/lpm.rc-temp
 	mv /tmp/extracted/lpm.rc-temp /tmp/extracted/lpm.rc
...
cd /tmp/extracted/sbin; ln -s ../init ueventd; ln -s ../init watchdogd; ln -s healthd ../charger;
